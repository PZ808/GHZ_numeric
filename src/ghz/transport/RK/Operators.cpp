...
namespace ghz {
    ...
    XDerivs GHPDifferX::diff_solution(
        ...
        D.PpHr_X = GHPSpectral(Nr, Nz, {m, kr, kz}, GHPScalar<Complex>(teuk::zeroC, p - 1, q - 1));
        D.PpHr_P_X = GHPSpectral(Nr, Nz, {m, kr, kz}, GHPScalar<Complex>(teuk::zeroC, p, q));
...
#pragma omp parallel for default(none) shared(p, q, D, held_ops, Nr)
        for (size_t ir = 0; ir < Nr; ++ir) {
            ...
            auto slice_PpH_X = D.PpHr_X.slice_R(ir);
            auto slice_PpH_P_X = D.PpHr_P_X.slice_R(ir);
            ...
            // fix this because the second third argument should be the second derivative
            held_ops.thornPHr_inplace_RSliceV(tetrad_, slice_P_X, slice_P_X,
                                              slice_PpH_P_X);
            ...
            held_ops.edthBarH_inplace_RSliceV(slice_EH_X, slice_EbH_EH_X);
