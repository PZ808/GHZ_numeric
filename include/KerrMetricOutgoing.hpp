...
public:
    ...
    void build_at(const OutgoingCoords Xout);

    inline Real a() const { return params.a; }

    teuk::SymmetricMatrix4 g(const OutgoingCoords Xout) const;
    teuk::SymmetricMatrix4 ginv(const OutgoingCoords Xout) const;

    void compactify(const OutgoingCoords Xout);

    void build_compact(const OutgoingCoords Xout);

    teuk::SymmetricMatrix4 g_tilde(const OutgoingCoords Xout) const;

    void build_compact_from_outgoing(const OutgoingCoords Xout);

    void build_compact(const OutgoingCoordsCompact Xout);

    teuk::SymmetricMatrix4 g_tilde(const OutgoingCoordsCompact Xout) const;
